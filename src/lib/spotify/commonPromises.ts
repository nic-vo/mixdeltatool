...
export async function localTimeout<D>(parentTimeout: number): Promise<D> {
	await new Promise(async (r) =>
		setTimeout(() => r, parentTimeout - Date.now())
	);
...
export async function getOnePromise(params: {
	...
	while (true) {
		try {
			...
			if (networkRetry === false) throw new FetchError();
			networkRetry = false;
			continue;
		}
		networkRetry = true;
	...
	if (errorOverrides)
		for (const custom of errorOverrides)
			if (response.status === custom.status) throw custom;
	switch (response.status) {
		...
		default:
			throw new FetchError("There's something wrong with Spotify");
	}
}
